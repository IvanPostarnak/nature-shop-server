...
class PurchaseController extends Controller {
  ...
  async getByQuery(queryObj, infoMod) {
    ...
    try {
      response.body = await this.database.getByQuery(filteredQuery, 'purchases', infoMod);
      response.count = response.body.length;
      if (response.count === 0) {
        response.code = 404;
        response.body = `Purchases by queries : '${JSON.stringify(filteredQuery)}' were not found`;
      }
    } catch (err) {
      response.code = 500;
  ...
  async getSupport(queryObj, param) {
    ...
    const filteredQuery = filterObjectByValue(queryObj);
    try {
      response.body = await this.database.getByQuery(filteredQuery, 'purchases', param);
      response.count = response.body.length;
      if (response.count === 0) {
        response.code = 404;
